...
public class BillingDataSource implements
    ...
    public static void setYmnCallback(YmnCallback ymnCallback) {
        BillingDataSource.ymnCallback = ymnCallback;
    }
    ...
    public static BillingDataSource getInstance(
        ...
        if (sInstance == null) {
            synchronized (BillingDataSource.class) {
                if (sInstance == null) {
                    sInstance = new BillingDataSource(context, knownInappSKUs, knownSubscriptionSKUs, autoConsumeSKUs);
                }
            }
        }
        return sInstance;
    ...
    public void onBillingSetupFinished(BillingResult billingResult) {
        ...
        switch (responseCode) {
            case BillingClient.BillingResponseCode.OK:
                ...
                ymnCallback.onCallBack(YmnCode.PAYRESULT_INIT_SUCCESS, debugMessage);
                break;
            default:
                retryBillingServiceConnectionWithExponentialBackoff();
                ymnCallback.onCallBack(YmnCode.PAYRESULT_INIT_FAIL, debugMessage);
                break;
        }
    }
    ...
    private void retryBillingServiceConnectionWithExponentialBackoff() {
        handler.postDelayed(() -> billingClient.startConnection(BillingDataSource.this), reconnectMilliseconds);
    ...
    /**
     * This is a single live event that observes consumed purchases from calling the consume method.
     *
    ...
    private void canPurchaseFromSkuDetailsAndPurchaseLiveData
        ...
        if (null == skuDetailsLiveData.getValue()) {
            ...
            result.setValue(null == skuState || skuState == SkuState.SKU_STATE_UNPURCHASED);
        }
    }
                ...
                if (purchaseState == Purchase.PurchaseState.PURCHASED) {
                    ...
                    for (String sku : purchase.getSkus()) {
                        if (knownAutoConsumeSKUs.contains(sku)) {
                            ...
                            if (isConsumable) {
                                Log.e(TAG, "Purchase cannot contain a mixture of consumable and non-consumable items: " + purchase.getSkus().toString());
                                isConsumable = false;
                                break;
                            }
                        }
